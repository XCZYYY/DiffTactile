...
def main():
    ...
    for opts in range(num_opt_steps):
        ...
        legacy_dir = f"lr_surface_follow_state_{args.use_state}_tactile_{args.use_tactile}_{args.times}"
        ...
        if opts %5 == 0 or opts == num_opt_steps-1:
            ...
            np.save(os.path.join(legacy_dir, f"control_p_gripper_{opts}.npy"), contact_model.p_sensor1.to_numpy())
            np.save(os.path.join(legacy_dir, f"control_o_gripper_{opts}.npy"), contact_model.o_sensor1.to_numpy())
