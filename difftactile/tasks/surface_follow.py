...
def main():
    ...
    if record_video:
        video_recorder = VideoRecorder(
            run_layout.videos / "surface_follow.mp4",
            fps=args.video_fps,
            frame_size=(960, 720),
            mirror_paths=[run_layout.mirror_videos / f"{run_layout.run_id}_surface_follow.mp4"],
        )
    ...
    def render_gui(frame_idx, stage="Forward", step=0, total_steps=None):
        ...
        viz_offset = [0.28, 0.08] if presentation else [0.5, 0.2]
    ...
    def run_replay_demo():
        print("Running surface_follow replay presentation")
        contact_model.init_pos_control()
        pos, ori = apply_single_sensor_replay_controls(contact_model, "surface_follow")
        ...
        for _ in range(max(1, int(args.demo_warmup_frames))):
            render_gui(0, "Ready: curved surface, dome, tactile maps", 0, total_replay_steps)
            maybe_sleep(delay)
        ...
        for loop_idx in range(loops):
            ...
            for ts in range(num_total_steps - 1):
                ...
                for ss in range(num_sub_steps - 1):
                    ...
                    render_gui(ss, f"Replay loop {loop_idx + 1}/{loops}: press and slide", global_step, total_replay_steps)
                    maybe_sleep(delay)
                    global_step += 1
        ...
        plt.savefig(run_layout.plots / "surface_follow_replay_controls.png")
        plt.close()
    ...
    for opts in range(num_opt_steps):
        ...
        if opts %5 == 0 or opts == num_opt_steps-1:
            ...
            plt.plot(losses)
            plot_name = f"surface_follow_{args.use_state}_tactile_{args.use_tactile}_{opts}.png"
            plt.savefig(run_layout.plots / plot_name)
            plt.close()
            np.save(run_layout.trajectories / f"control_p_gripper_{opts}.npy", contact_model.p_sensor1.to_numpy())
            np.save(run_layout.trajectories / f"control_o_gripper_{opts}.npy", contact_model.o_sensor1.to_numpy())
