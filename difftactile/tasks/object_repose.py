...
def main():
    ...
    reject_inline_record_video(bool(args.record_video), "object_repose")
    ...
    def render_gui(frame_idx, stage="Forward", step=0, total_steps=None):
        ...
        viz_scale = 0.22 if presentation else 0.1
        viz_offset = [0.18, 0.12] if presentation else [0.0, 0.0]
    ...
    def run_replay_demo():
        print("Running object_repose replay presentation")
        contact_model.init_pos_control()
        pos, ori = apply_single_sensor_replay_controls(contact_model, "object_repose")
        ...
        for _ in range(max(1, int(args.demo_warmup_frames))):
            render_gui(0, "Ready: object, tactile dome, marker force map", 0, total_replay_steps)
            maybe_sleep(delay)
        ...
        for loop_idx in range(loops):
            ...
            for ts in range(num_total_steps - 1):
                ...
                for ss in range(num_sub_steps - 1):
                    ...
                    render_gui(ss, f"Replay loop {loop_idx + 1}/{loops}: push to repose", global_step, total_replay_steps)
                    maybe_sleep(delay)
                    global_step += 1
        ...
        plt.savefig(run_layout.plots / "object_repose_replay_controls.png")
        plt.close()
